import { Arrow, HandNumber, Star } from "@/components/Doodles";
    ...
    <section id="process" className="relative px-4 md:px-8 py-24 md:py-36 bg-acid border-y-[3px] border-ink overflow-hidden">
      <div className="max-w-[1300px] mx-auto">
        <div className="text-center mb-16">
          <span className="font-hand text-2xl flex items-center justify-center gap-2">
            <Star size={22} /> how it works <Star size={22} />
          </span>
          <h2 className="text-6xl md:text-8xl mt-2">
            Three moves, <span className="italic font-display">no fluff.</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-10 md:gap-6 relative">
          {steps.map((s, i) => (
            <div key={s.n} className="relative">
              ...
              {i < steps.length - 1 && (
                <Arrow
                  className="hidden md:block absolute top-1/2 -right-6 -translate-y-1/2 text-ink z-10"
                  size={80}
                />
              )}
            </div>
